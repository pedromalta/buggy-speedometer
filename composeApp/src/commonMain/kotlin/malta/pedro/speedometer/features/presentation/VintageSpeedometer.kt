    ...
    Box(
        ...
        Canvas(modifier = Modifier.fillMaxWidth().height(400.dp)) {
            ...
            for (i in 0..<majorTickCount) {
                val angle = (startAngle + (sweep / (majorTickCount - 1)) * i).toDouble() * PI / 180.0
                val inner = Offset(
                ...
                if (i % 2 == 0) {
                    ...
                    val tx = center.x + (radius * 0.46f) * cos(angle).toFloat() - textLayout.size.width / 2
                    val ty = center.y + (radius * 0.46f) * sin(angle).toFloat() - textLayout.size.height / 2
